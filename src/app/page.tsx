...
export default function Home() {
  ...
  const [health, setHealth] = useState<HealthState>({ checked: false, configured: true, missing: [] })

  const checkSession = useCallback(async () => {
    try {
      // Diagnóstico: ¿el servidor tiene Supabase configurado?
      try {
        const hRes = await fetch('/api/health')
        const hData = await hRes.json()
        setHealth({
          checked: true,
          configured: Boolean(hData.supabase_configurado),
          missing: Array.isArray(hData.faltantes) ? hData.faltantes : [],
        })
      } catch {
        // Si /api/health no responde, no bloqueamos el login
        setHealth({ checked: true, configured: true, missing: [] })
      }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setLoginError(data.error || `Error del servidor (código ${res.status})`)
        return
      }
      setUser(data.user)
    } catch {
      setLoginError('Error de conexión: no se pudo contactar al servidor. Verifica que el sitio esté desplegado correctamente.')
    } finally {
        ...
        <Card className="w-full max-w-md shadow-2xl border-white/10 bg-white/95 backdrop-blur-sm">
          ...
          <CardContent>
            <form onSubmit={handleLogin} className="space-y-4">
              {health.checked && !health.configured && (
                <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-3 text-sm text-amber-900">
                  <p className="font-semibold mb-1">⚠ Servidor sin configurar</p>
                  <p className="leading-relaxed">
                    Faltan variables de entorno en el servidor de despliegue:
                    <span className="font-mono text-xs"> {health.missing.join(', ')}</span>.
